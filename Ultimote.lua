			...
			local handled = false
				for j = 1, #objects do
					...
					if( x > bounds.xMin and x < bounds.xMax and y > bounds.yMin and y < bounds.yMax) then
						event.target = object
						if( ( (object.isVisible and object.parent.isVisible) or object.isHitTestable) and object:dispatchEvent( event )) then handled = true; break;	end
					end
				end
				end
					if(not handled) then
...
local function generateBoundingBoxes()
	...
	for j = #objects, 1,-1 do
		...
		local object  = objects[j]
		if(object.parent) then
		if((options.sendAllObjects or object.ultimoteObject) and object.isVisible and object.parent.isVisible) then
